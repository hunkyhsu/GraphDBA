...
from graphdba.agents.state import AgentState, NodeName, AgentWorkflowStatus
...
def route_diagnostic(state: AgentState) -> str:
    ...
    status: AgentWorkflowStatus = state["workflow_status"]
    if status == AgentWorkflowStatus.DIAGNOSED:
...
def route_validation(state: AgentState) -> str:
    """Where to go AFTER Diagnostic Node"""
    status: AgentWorkflowStatus = state["workflow_status"]
    if status == AgentWorkflowStatus.VALIDATED_SUCCESS:
        return NodeName.PLANNING
    elif status == AgentWorkflowStatus.VALIDATED_FAIL:
...
def build_graph(
    ...
    # 2. edges
    builder.add_edge(START, NodeName.DIAGNOSTIC)
